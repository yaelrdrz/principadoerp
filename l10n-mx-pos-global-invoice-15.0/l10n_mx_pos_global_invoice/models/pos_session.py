...
class PosSession(models.Model):
    ...
    def _create_account_move(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        ...
        if self.config_id.create_global_invoice and \
            ...
            _logger.info("Creating account move in automatic mode")
            ...
            if not self.config_id.global_customer_id:
                raise UserError(_('Global customer not detected, please configure global customer on pos config'))
    ...
    def createStockMove(self, name, product_id, quantity):
        ...
        stock_move.move_line_ids.write({'qty_done': quantity})  # Crea un record stock.move.line
        ...
        for order in self.order_ids:
            ...
            if not order_is_invoiced:
                ...
                for order_line in order.lines:
                    ...
                    # self.createStockMove(order_line.product_id.id, order_line.product_id.id, order_line.qty) TODO: Uncomment
    ...
    def _validate_session(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        if self.config_id.create_global_invoice and self.config_id.global_invoice_method == 'manual':
            journal = self.config_id.journal_id
            self.ensure_one()
            ...
            return True
        elif self.config_id.global_invoice_method == 'automatic':
            journal = self.config_id.journal_id
            self.ensure_one()
            self._check_if_no_draft_orders()
            account_move = self.env['account.move'].search([
            ...
            if self.move_id.line_ids:
                ...
                self.env['pos.order'].search([('session_id', '=', self.id), ('state', '=', 'paid')]).write(
                    {'state': 'done'})
            self.write({'state': 'closed'})
            self.create_global_invoice(journal, self.config_id.global_customer_id, self)
            return True
        else:
            res = super(PosSession, self)._validate_session()
            if self.global_invoice_id and self.config_id.create_global_invoice \
    ...
    def create_global_invoice(self, journal, global_customer, pos_session, bank_payment_method_diffs=None):
        ...
        global_invoice = self.env['account.move'].with_context(default_journal_id=journal.id).create({
            'journal_id': journal.id,
            'date': fields.Date.context_today(self),
            'ref': pos_session.name,
        ...
        if account_move.line_ids:
            ...
            self.env['account.move.line'].search([(
                'move_id', '=', account_move.id),
                ('check_global_invoice', '=', False)]).unlink()
            account_move._post()
        ...
        pos_session.write({'has_global_invoice': True})
