...
sock = xmlrpclib.ServerProxy('https://principado.odoo.com/xmlrpc/2/object')
row = 0

def generate_record(vals,last_parent):
	last_categ_id = sock.execute_kw(dbname, uid, password, 'product.public.category', 'create', [{
		    'name': vals
		}])
	if last_parent and last_categ_id:
		sock.execute_kw(dbname, uid, password, 'product.public.category', 'write', [[last_parent], {
