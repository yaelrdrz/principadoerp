...
sock = xmlrpclib.ServerProxy('http://localhost:8069/xmlrpc/2/object'.format(url))
row = 0

def generate_record(vals,last_parent):
	last_categ_id = sock.execute_kw(dbname, uid, password, 'product.category', 'create', [{
		    'name': vals
		}])
	if last_parent and last_categ_id:
		sock.execute_kw(dbname, uid, password, 'product.category', 'write', [[last_parent], {
...
with open('/home/odoo/src/user/import_product_excel_cr/principado_product/final_catalogue_141022_latest.csv', mode='r')as file:
	...
	for lines in csvfile:
		...
		print (' productname, id,categ id  =================',prod_name, row)
		product_id and sock.execute_kw(dbname, uid, password, 'product.template', 'write', [[product_id[0]], {
		    'categ_id': final_dict[ ''.join(actual_lines[10:20]) ]
		}])
		row += 1
